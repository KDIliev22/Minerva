...
/**
 * Extended MldhtService that persists the DHT routing table to a stable
 * directory instead of a throwaway temp dir. This dramatically speeds up
 * DHT bootstrap on subsequent startups.
 */
public class PersistentMldhtService implements DHTService {
    private static final Logger LOGGER = LoggerFactory.getLogger(PersistentMldhtService.class);
    ...
    @Inject
    public PersistentMldhtService(IRuntimeLifecycleBinder lifecycleBinder, Config config, DHTConfig dhtConfig,
    ...
    /**
     * Creates a DHTConfiguration that persists the routing table to a stable
     * directory (./dht) and preserves the node ID across restarts.
     */
    private DHTConfiguration toMldhtConfig(DHTConfig dhtConfig) {
        // Use a fixed directory for DHT state persistence
        Path storagePath = Path.of(System.getProperty("user.dir"), "dht");
        File storageDir = storagePath.toFile();
        if (!storageDir.exists()) {
            storageDir.mkdirs();
        }
        LOGGER.info("DHT state persistence directory: {}", storagePath);
        ...
        return new DHTConfiguration() {
            ...
            public boolean isPersistingID() {
                return true; // Persist node ID across restarts
            }

            @Override
            public Path getStoragePath() {
                return storagePath; // Stable directory, not a temp dir
            }
            ...
            public Predicate<InetAddress> filterBindAddress() {
                return address -> {
                    ...
                    if (LOGGER.isDebugEnabled()) {
                        LOGGER.debug("Filtering addresses to bind DHT server to.. Checking " + address
                                + ".. Could use: " + couldUse);
                    }
    ...
    private synchronized void start() {
        if (started.compareAndSet(false, true)) {
            try {
                ...
                mapPorts();
                LOGGER.info("DHT started with persistent routing table");
    ...
    private synchronized void shutdown() {
        if (started.compareAndSet(true, false)) {
            LOGGER.info("Shutting down DHT, persisting routing table...");
            dht.stop();
        }
    }
